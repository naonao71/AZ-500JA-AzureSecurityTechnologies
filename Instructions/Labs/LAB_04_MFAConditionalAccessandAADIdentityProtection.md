...
# ラボ 04: MFA、条件付きアクセス、AAD Identity Protection
# 受講生用ラボ マニュアル
...
1. **「上記の使用条件に同意する」** チェックボックスをクリックして、**「購入」** をクリックします。
    ...
    >**注**: デプロイメントが完了するのを待たず、次の演習に進みます。このラボの最後の演習では、このデプロイに含まれる仮想マシンを使用します。
...
1. 「**テナントの作成**」 ブレードの 「**基本**」 タブで、オプション 「**Azure Active Directory**」 が選択されていることを確認し、「**次: 構成**」 をクリックします。
...
1. Azure AD の管理画面に移動します。

1. **「ユーザー」 \| 「すべてのユーザー」** ブレードで、ユーザー アカウントを表すエントリをクリックします。 

1. 自身のユーザー アカウント（Microsoft アカウント）を選択してプロパティを表示し、、「**編集**」 をクリックします。  
...
1. 「**ライセンスの割り当て**」 ブレードで、「**ユーザーとグループ**」 をクリックします。
...
1. メッセージが表示されたら、「**アカウントの保護にご協力ください**」 ダイアログ ボックスで 「**次へ**」 をクリックします。   

    >**注**: ブラウザー セッションは、「**アカウントのセキュリティ保護**」 ページにリダイレクトされます。

1. 画面の下にある **別の方法を設定します** をクリックし、、次のいずれかのオプションを設定する必要があることに注意してください。

   - **電話**

   - **認証アプリ**

1. 「**電話**」 を選択し、**確認** をクリックしてください。

1. **電話** 画面で、使用可能な携帯電話の番号を入力して、「**次へ**」をクリックします。

1. 指定した電話にコードが通知されるので、コードを転記し、「**次へ**」をクリックします。

1. **SMSが検証されました。お使いの電話が正常に登録されました** と表示されていることを確認後、「**次へ**」をクリックします。

1. 「**別の方法を設定します**」を選択し、一覧から「**電子メール**」を選択して「**確認**」をクリックします。

1. 使用可能な電子メールアドレスを指定します。

1. 電子メールアドレス宛に届いたコードを転記して「**次へ**」をクリックします。
...
1. 「**AdatumLab500-04**」 ブレードの 「**管理**」 セクションで、「**プロパティ**」を選択し、画面下にある「**セキュリティ規定値群の管理**」をクリックしてください。

1. **セキュリティの規定値群の有効化** を **いいえ** に設定し、**自分の組織では条件付きアクセスを使用している** をチェックして **保存** をクリックします。
...
1. プロンプトが表示されたら、「**詳細情報が必要です**」 ダイアログ ボックスで 「**次へ**」 をクリックします。   

    >**注**: ブラウザー セッションは、**アカウントのセキュリティ保護**のページにリダイレクトされます。

1. **別の方法を設定します** をクリックし、次のいずれかのオプションを設定する必要があることに注意してください。

   - **電話**

   - **認証アプリ**

1. **電話** を選択して **確認** をクリックします。

1. セクションのドロップダウンリストで国または地域を選択し、空のテキストボックスに携帯電話番号を入力して、「**次へ**」 をクリックします。

1. テキストボックスに携帯電話のテキスト メッセージで受け取ったコードを転記して、「**次へ**」 をクリックします。   

1. **SMSが検証されました** が表示されたら **次へ** をクリックします。

1. **成功** 画面が表示されたら **完了** をクリックします。
...
- タスク 1: Azure portal で Azure AD Identity Protection オプションを表示する
...
1. 「**保護**」 セクションの 「**ユーザー リスク ポリシー**」 をクリックします。

1. 次の設定を使用して、**ユーザー リスクの修復ポリシー**を構成します。 

   - 「**ユーザー**」 をクリックし、「**ユーザー**」 ブレードの 「**含める**」 タブで、「**全てのユーザー**」 オプションを選択します。 

   - 「**ユーザー**」 ブレードで 「**除外**」 タブに切り替え、「**除外されたユーザーを選択**」 をクリックします。**自分自身の** ユーザー アカウントを選択して 「**選択」 をクリックし、「**完了**」 をクリックします。 

   - 「**ユーザーリスク**」 をクリックします。「**ユーザー リスク**」 ブレードで 「**低以上**」 を選択して 「**完了**」 をクリック します。 

   - 「**割り当て | アクセス**」 をクリックし 、「**アクセス**」 ブレードで 「**アクセスを許可**」 オプションと 「**パスワードの変更が必要**」 チェック ボックスがオンになっていることを確認し、「**完了**」 をクリックします。
...
1. 「**保護**」 セクションで、「**サインイン リスク ポリシー**」 をクリックします

1. **サインイン リスク是正ポリシー**を次の設定で構成します。  

   - 「**ユーザー**」 をクリックし、「**ユーザー**」 ブレードの 「**含める**」 タブで、「**全てのユーザー**」 オプションを選択します。 

   - 「**サインイン リスク**」 をクリックし 「**中以上**」 を選択し、「**完了**」 をクリックします。 
...
1. RDP セッションを閉じます。 
    ...
    >**注**: この時点で、2 つの異なるサインインを試みました。次に、Azure Identity Protection レポートを確認します。
...
**AdatumLab500-04** Azure AD テナントの Identity Protection ポリシーを無効にするには、次の手順を使用します。
...
1. 「**Identity Protection**」 \| 「**ユーザー リスク ポリシー**」 ブレードで 「**ポリシーの適用**」 を 「**オフ**」 に設定し、「**保存**」 をクリックします。

1. 「**Identity Protection**」 \| 「**ユーザー リスク ポリシー**」 ブレードで、「**サインイン リスク ポリシー**」  をクリックします

1. 「**Identity Protection**」 \| 「**サインイン リスク ポリシー**」 ブレードで 「**ポリシーの適用**」 を 「**オフ**」 に設定し、「**保存**」 をクリックします。      

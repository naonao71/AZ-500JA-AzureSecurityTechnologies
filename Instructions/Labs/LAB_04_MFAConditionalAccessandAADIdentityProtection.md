...
1. 「**AdatumLab500-04** Azure Active Directory」 ブレードの「**管理**」 セクションで、「**ライセンス**」 をクリックします。
...
1. 「**ユーザーとグループの追加**」 ブレードで、 **aaduser1**、**aaduser2**、 **aaduser3**、のユーザー アカウントを選択し、「**選択**」 をクリックします。

1. 「**ライセンスの割り当て**」 ブレードで、「**割り当てオプション**」 をクリックし、すべてのオプションが有効になっていることを確認して、「**レビューと割り当て**」 をクリックします。

1. 「**ライセンスの割り当て**」 ブレードで、「**割り当て**」 をクリックします。
...
1. 「**アカウントのセキュリティ保護**」 ページで、ブラウザを閉じます。

   >**注**: Microsoft Authenticator アプリの設定はこの演習では省略しています。

1. InPrivate ブラウザーの画面を開きます。

1. Azure portal に移動し、**aaduser1** ユーザー アカウントを使用してサインインします。 

1. 認証コードを入力し、パスワードを変更します。新しいパスワードを必ず記録してください。
...
1. **新規**のブレードで、次の設定を構成します。
   ...
   - 「**ユーザーとグループ**」 をクリックし、「**ユーザーとグループの選択**」 チェックボックスを選択し、「**ユーザーとグループ**」をチェックします。
   
   - 「**選択**」 ブレードで、「**aaduser2**」 をクリックして、「**選択**」 をクリックします。
	
   - 「**クラウド アプリまたは操作**」 をクリックし、「**クラウドアプリ**」をドロップダウンリストから選択し、対象タブの **アプリを選択** をチェックします。
   
   - 「**選択**」 ブレードで 「**Microsoft Azure Management**」 をクリックして、 「**選択**」 をクリックします。 
...
1. 次の設定を使用して、**ユーザー リスクの修復ポリシー**を構成します。 
   ...
   - 「**ユーザー**」 をクリックし、「**ユーザー**」 ブレードの 「**対象**」 タブで、「**すべてのユーザー**」 オプションを選択します。

   - 「**ユーザーのリスク**」 をクリックし、「**ユーザーのリスク**」 ブレードで、「**低以上**」 を選択し、「**完了**」 をクリックします。 

   - 「**アクセス**」 をクリックし 、「**アクセス**」 ブレードで 「**アクセスを許可**」 オプションと 「**パスワードの変更が必要**」 チェック ボックスがオンになっていることを確認し、「**完了**」 をクリックします。

   - 「**ポリシーを適用する**」 を 「**オン**」 に設定し、「**保存**」 をクリック します。
...
1. **サインイン リスク是正ポリシー**を次の設定で構成します。 
   ...
   - 「**アクセス**」 をクリックし、「**アクセス**」 ブレードで 「**アクセスを許可**」 オプションと 「**多要素認証を要求する**」 チェックボックスがオンになっていることを確認し、「**完了**」 をクリックします。
...
1. 検証を完了し、アプリケーション アクセス パネルに正常にサインインしたことを確認します。
    ...
    >**注**: Tor Browser の Extension が起動した場合は、リクエストを許可します。

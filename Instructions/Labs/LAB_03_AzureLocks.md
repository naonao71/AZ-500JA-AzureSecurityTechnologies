...
1. 「**OK**」 をクリックします。 
   ...
   >**注意**:  ストレージ アカウントは、誤って削除および変更されないように保護されるようになりました。
...
1. 次のテキストに似た通知を確認します。 
   ...
   >**注意**:  **削除**ロックによって構成の変更が許可され、誤って削除されないことが確認されました。

   >**注意**:  リソース ロックを使用することにより、最も重要なリソースの偶発的または悪意のある変更や削除に対する追加の防御ラインを実装できます。**所有者**ロールを持つユーザーは、リソース ロックを削除できますが、その場合は意識的な作業が必要です。ロックはロール ベースの Access Control を補足します。 

...
1. 「Log Analytics ワークスペース」ブレードの「**データ ソースの接続**」セクションで、「**Virtual Machines**」エントリをクリックします。
...
1. 「**Windows イベントログ**」が選択されていることを確認し、「**+ Windows イベントログを追加**」をクリックします。イベントログの種類のリストで、「**システム**」を選択します。

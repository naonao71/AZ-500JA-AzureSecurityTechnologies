...
1. 「**カスタム デプロイ**」 ブレードの 「**デプロイの範囲**」 で、次の設定が構成されていることを確認します (既定値を使用して他の設定を行います)。
   ...
   |Admin Password|**独自パスワード（後で使用するため記録しておきます）**|
...
1. キー コンテナー で、「**アクセス ポリシー**」 をクリックし、「**作成**」 をクリックします。
...
1. 「**アクセス ポリシーの追加**」 ブレードで、次の設定を指定します (他の設定はすべて既定値のままにします)。 
    ...
    |キーのアクセス許可|「**キーの管理操作**」 と「**暗号化操作**」 ですべて選択して、**16 個**の権限を選択します（特殊キー操作とポリシーのローテーション操作はチェックしません）|
...
1. 「**プリンシパル**」で、**自身のID** をクリックしてアクセス ポリシーを作成します。
...
1. **ADO.NET 接続文字列**を記録します。この情報は後で必要になります。
    ...
    >**注**: 接続文字列を使用する場合は、必ず `{your_password}` プレースホルダーを「**独自パスワード**」に置き換えてください。
...
1. **パブリックアクセス** タブの**ファイアウォール規則** セクションから**「ファイアウォールルールの追加」**をクリックして次の設定を指定します。 

...
1. 「**Azure アクティビティ**」ブレードで、**手順** タブの「Azure Plolicy の割り当てウィザードの起動と手順の実行。」を確認します。この手順に従って Azure Policy を構成するため、「**Azure Policy の割り当てウィザードの起動**」リンクをクリックします。

    >**注**: ポリシーの割り当てスコープには、利用しているサブスクリプションを指定します。修復タブで「**修復タスクを作成する**」にチェックを入れます。あとは規定の設定を受け入れます。

    >**注**: ポリシーが適用されるまで5分程度かかります。その後、自動的に接続されます。
...
1. 「**Azure アクティビティ**」ブレードで、「**次の手順**」タブをクリックします。 

1. 「**関連する分析テンプレート**」一覧から、「**Suspicious number of resource creation or deployment activities**」テンプレートの「**ルールの作成**」をクリックします（必要であればページの右側にスクロールする）。 

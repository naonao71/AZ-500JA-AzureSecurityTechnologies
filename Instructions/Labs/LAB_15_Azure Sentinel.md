...
1. **Azure Sentinel **の **「分析」** ブレードで、**「規則のテンプレート」** タブをクリックします。 

    >**注**: 作成できるルールの種類を確認します。各ルールは、特定のデータ ソースに関連付けられます。

1. **「規則のテンプレート」**内の **Azure アクティビティ** データ ソースに関連付けられたテンプレートの **「Suspicious number of resource creation or deployment（不審なリソースの作成またはデプロイの数）」** エントリをクリックして、テンプレートのプロパティが表示されているペインで、**「ルールの作成」** をクリックします。
